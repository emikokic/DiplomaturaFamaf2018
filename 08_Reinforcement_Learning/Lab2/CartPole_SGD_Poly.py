...
class SGDPolyCartPoleSolver:
    def __init__(self, n_episodes=1000, max_env_steps=None,
                 gamma=0.9, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.005,
        ...
        # Whether or not to display video
        if monitor:
        ...
        self.model.partial_fit(
            self.preprocess_state(self.env.reset(), 0), [0]
        )
    ...
    def choose_action(self, state, epsilon):
        ...
        """
        Exploits the current knowledge if the random number > epsilon,
        otherwise explores
        """
    ...
    def get_epsilon(self, episode):
        """
        Returns an epsilon that decays over time until a minimum epsilon value
        is reached; in this case the minimum value is returned"""
        return max(
            self.epsilon_min,
    ...
    def preprocess_state(self, state, action):
        """
        State and action are stacked horizontally and its features are
        combined as a polynomial to be passed as an input of the approximator
        """

        """
        poly_state converts the horizontal stack into a combination of its
        parameters i.e.:
    ...
    def replay(self, batch_size):
        """
        Previously stored (s, a, r, s') tuples are replayed
        (that is, are added into the model).
        The size of the tuples added is determined by the batch_size parameter
        """
        ...
        for state, action, reward, next_state, done in minibatch:
            qsa_s_prime = np.asarray(
                [self.model.predict(self.preprocess_state(next_state, a))
                 for a in range(self.env.action_space.n)]
            )
            ...
            if done:
                ...
                qsa_s = reward + self.gamma * np.max(qsa_s_prime)
    ...
    def run(self):
        ...
        for e in range(self.n_episodes):
            ...
            # t counts the number of time-steps the pole has been kept up
            t = 0
            while not done:
            ...
            if e % 100 == 0:
                print(
                    "[Episode {}] - Mean survival time over "
                    "last 100episodes was {} ticks."
                    .format(e, mean_score)
                )
